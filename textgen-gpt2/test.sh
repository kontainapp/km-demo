...
until $(curl --output out.json --silent --fail -XGET -H "Content-Type: application/json" -H "Accept: application/json" http://localhost:5000/predict -d '{"content":"In the morning"}'); do
   sleep 0.001
done
